$(function() {
	...
	var pageSize = 3;
	// 列出商品列表的URL
	var listUrl = '/bytetubeapp/frontend/listproductsbyshop';
	// 默认的页码
	var pageNum = 1;
	// 从地址栏里获取ShopId
	...
	var searchDivUrl = '/bytetubeapp/frontend/listshopdetailpageinfo?shopId=' + shopId;
	...
	//给兑换礼品的a标签赋值兑换礼品的URL，
	$('#exchangelist').attr('href', '/bytetubeapp/frontend/awardlist?shopId=' +
	shopId);
	// 获取本店铺信息以及商品类别信息列表
			...
			if (data.success) {
				...
				data.productList.map(function(item, index) {
					html += '' + '<div class="card" data-product-id='
							...
							+ 'update</p>' + '<span>click to check</span>' + '</div>'
							+ '</div>';
				});
				// 将卡片集合添加到目标HTML组件里
