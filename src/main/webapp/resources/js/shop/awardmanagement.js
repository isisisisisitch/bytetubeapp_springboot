$(function() {
	// 获取该店铺下的奖品列表URL
	var listUrl = '/bytetubeapp/shopadmin/listawardsbyshop?pageIndex=1&pageSize=9999';
	// 设置奖品的可见状态
	var changeUrl = '/bytetubeapp/shopadmin/modifyaward';
	getList();
	function getList() {
		// 访问后台，获取奖品列表
			...
			.on(
					...
					function(e) {
						...
						if (target.hasClass('edit')) {
							...
							window.location.href = '/bytetubeapp/shopadmin/awardoperation?awardId='
							...
							window.location.href = '/bytetubeapp/frontend/awarddetail?awardId='
									+ e.currentTarget.dataset.id;
						}
					});
	...
	// 给新增按钮绑定点击事件
	$('#new').click(function() {
		window.location.href = '/bytetubeapp/shopadmin/awardoperation';
	});
