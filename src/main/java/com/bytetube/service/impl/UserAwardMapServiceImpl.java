...
import com.bytetube.dao.UserAwardMapDao;
import com.bytetube.dao.UserShopMapDao;
import com.bytetube.dto.UserAwardMapExecution;
import com.bytetube.entity.UserAwardMap;
import com.bytetube.entity.UserShopMap;
import com.bytetube.enums.UserAwardMapStateEnum;
import com.bytetube.exceptions.UserAwardMapOperationException;
		...
		if (userAwardMap != null && userAwardMap.getUser() != null && userAwardMap.getUser().getUserId() != null
			...
			try {
				...
				if (userAwardMap.getPoint() != null && userAwardMap.getPoint() > 0) {
					...
					// 判断该用户在店铺里是否有积分
					if (userShopMap != null) {
						// 若有积分，必须确保该用户在店铺下积分大于本次要兑换奖品需要的积分
